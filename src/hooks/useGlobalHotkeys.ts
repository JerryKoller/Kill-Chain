...
/**
 * Globally bound keyboard shortcuts. Skips events that originate inside
 * a text input so renaming a preset doesn't toggle play, etc.
 */
export const HOTKEYS = [
  ...
  { keys: ["N", "P"], label: "Next / previous track in queue" },
  ...
  { keys: ["A"], label: "Snapshot A (current tuning)" },
  { keys: ["B"], label: "Swap A <-> B" },
  { keys: ["C"], label: "Clear A snapshot" },
  { keys: ["E"], label: "Toggle XM6 / headphone correction" },
  { keys: ["F"], label: "Toggle full-effect bypass" },
  { keys: ["S"], label: "Save current tuning as preset" },
  { keys: ["Z"], label: "Undo last tweak" },
  { keys: ["Shift+Z"], label: "Redo" },
  { keys: ["W"], label: "Toggle mini-player mode" },
] as const;
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      switch (key.toLowerCase()) {
        ...
        case "a":
          audio.storeAB();
          ui.toast("Snapshot A locked");
          return;
        case "b":
          if (audio.abSnapshot) {
            audio.swapAB();
            ui.toast("Swapped A <-> B");
          } else {
            ui.toast("Snapshot A first (press A)");
          }
          return;
        case "c":
          audio.clearAB();
