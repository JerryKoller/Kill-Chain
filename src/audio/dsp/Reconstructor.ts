 ...
 *
 * All four are 0..1 amounts; at 0 the module is a transparent wire (dry
 * gain 1, all wet paths silent, no timers running).
 */
...
export const RESTORE_OFF: RestoreParams = { hf: 0, body: 0, decrunch: 0, hiss: 0 };

export function restoreActive(p: RestoreParams): boolean {
  return p.hf > 0.001 || p.body > 0.001 || p.decrunch > 0.001 || p.hiss > 0.001;
}
...
export class Reconstructor {
  ...
  constructor(ctx: BaseAudioContext) {
    ...
    this.input.connect(this.crunchFilter).connect(this.hissShelf).connect(this.output);
    ...
    this.input
      ...
      .connect(this.pHp2)
      .connect(this.pLp);
    this.pLp.connect(this.pGain).connect(this.output);
    ...
    this.input.connect(this.aSrcSum);
    ...
    this.input.connect(this.crunchSide).connect(this.crunchAnalyser);
    ...
    this.input.connect(this.hissSideHi).connect(this.hissHiAnalyser);
