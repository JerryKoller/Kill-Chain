---
{
  "id": "fire-module-enable-feedback-live",
  "title": "Fire Command module enable visual feedback (live)",
...
# Fire Command module enable visual feedback (LIVE)

This is the first LIVE production edit for the local runner. Investigate current `ModuleEnableToggle` styling yourself. Do not invent a redesign of Fire Command.

Pick exactly one conservative visual contrast change. Do not present Option A/B/C for the operator. After the proposal passes, apply that exact edit to `src/components/FireCommand/ModuleEnableToggle.tsx` only. Do not commit. Human visual acceptance is after the live diff.

## Allowed edit

Only `src/components/FireCommand/ModuleEnableToggle.tsx`. Do not edit stores, AudioEngine, DSP, electron, or package.json.
...
You cannot see the rendered app. Leave appearance for human review. Do not restyle in circles.
